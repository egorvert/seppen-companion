...
@tool("add_reaction_to_message")
def add_reaction_to_message(reaction_emoji: str) -> str:
    """
    Add an emoji reaction to the user's most recent message.
    Use this to express emotion or acknowledgment without sending text.
    Only use reactions sparingly for significant emotional moments.
    
    Args:
        reaction_emoji: The emoji to use as a reaction (e.g., "❤️", "👍", "😂")
        
    Returns:
        A confirmation message about the reaction
    """
    # This function will be handled by the reaction_node in the graph
    # The actual reaction execution happens in the graph pipeline
    return f"Scheduled reaction: {reaction_emoji}"
